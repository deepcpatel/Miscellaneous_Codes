Codes written by Me!

Platforms and Languages used :

• "8085_Assembly_Programming"        -> Assembly language for 8085 processor
• "ATmega_32_Programs"               -> Written in MikroC platform in C language
• "Bash_Programming"                 -> Linux Bash
• "Career_Recommendation_system"     -> Python 2.7 and 3
• "Computer_Networking_Programs"     -> Applications using network socket programming in C
• "Database_Management_Systems"      -> MySQL
• "Data_Structures_and_Algorithms"   -> C and Java
• "Digital_Signal_Processing_Codes"  -> GNU Octave/MATLAB
• "Elementary_C_C++_Programming"     -> C/C++ in Microsoft Windows (Turbo C compiler)
• "Generalized_Second_Price_Auction" -> Python 2.7
• "Linear_Algebra_Codes"             -> C++
• "Object_Oriented_Programming"      -> C++ and Java
• "Random_Number_Generation_Program" -> Python 2.7
• "Randomized_Closest_Point_Pairs"   -> C
• "Some_Data_Analysis_Algorithms"    -> GNU Octave/MATLAB
• "Video_Shot_Detection_using_ECR"   -> Python 2.7 using NumPy and OpenCV
