...
int pop(int *array, int &size, int &front, int &rear)		//pop function to delete from priority queue
{
	...
	else
	{
		...
		min_heapify(array, size, front, rear);
...
void min_heapify(int *array, int &size, int &front, int &i)
{
	int small, temp;
	int left=2*i;
	int right=(2*i)+1;
	...
	if(right<front && array[right]<array[small])
	{
		small=right;
	}
	...
	if(small!=i)
	{
		...
		min_heapify(array, size, front, small);
	}
}
