...
int main(int argc, char *argv[])
{
	...
	if((sockfd = socket(AF_INET, SOCK_DGRAM, 0)) == -1)
		...
		write(1,"\n Server : Socket creation successful",37); //initialize client_addr struct with 0s
	...
	if(bind(sockfd, (struct sockaddr*) &my_addr, sizeof(my_addr)) == -1)
		...
		write(1,"\n Client : bind successful\n",27);		// initialize serv_addr struct with 0s
	...
	if(sendto(sockfd, "Hi", 2, 0, (struct sockaddr*)&serv_addr, slen) ==-1)
